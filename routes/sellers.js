...
router.get('/', async function(req, res, next) {  
    try {
        ...
        if(rToken.status === 'ok' && rToken.id_user){  
            ...
            const qryCustomers = "SELECT * FROM web_buddy.sellers";
...
router.post('/', async function(req, res, next) {  
    ...
    if(validaToken.result.data === 'ok' ){     
        ...
        var registro={
            ...
            id_portal: req.body.id_portal, 
            quantity_articles: req.body.quantity_articles 
        };

        db.query('insert into web_buddy.sellers set ?',
            registro, 
